...
int main() {
    ...
    inp_text = get_text();
    wchar_t key = L'1';
    wprintf(L"Чтобы заменить цифры ,стоящие в конце слова, на буквы  \"1\".\n"
            "Чтобы вывести все предложения в которых на последнем месте, последнее слово, предыдущего предложения \"2\".\n"
            "Чтобы отсортировать предложения по уменьшению количества слов в предложении, и вывести полученный текст, введите \"3\".\n"
            "Чтобы удалить все предложения в которых меньше 3 слов, и вывести полученный текст, введите \"4\".\n"
